...
> **Статус 04.08 (задача W5).** Строка 7 ЗАКРЫТА. Хранение секретов тенанта уже было
> (`secure-storage.ts` + `tenants.ts`) — не хватало API снаружи, и из-за этого
> «собери отчёт по моему Директу» облачному пользователю был недостижим. Добавлены
> `GET /connectors` и `POST|DELETE /connectors/{id}/secrets` (`headless/server.ts`
> поверх `headless/connector-secrets.ts`), тенант из `X-Verstak-Tenant`, сервисный
> bearer как у остальных ручек. Наружу уходят только ИМЕНА ключей. Это отметка об
> ОДНОЙ строке: остальные строки таблицы этой датой не перепроверялись.
