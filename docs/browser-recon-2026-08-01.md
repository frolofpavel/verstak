...
> **ИСТОРИЧЕСКИЙ СНИМОК ДО ПРАВОК.** Многое из описанного ниже уже ЗАКРЫТО: браузер
> работает БЕЗ открытой вкладки (`a0f7a22` + гонка `b008403`, 02.08); структурный
> снимок с нумерацией, клик/ввод по номеру, ожидание, `browser_find`, чтение
> консоли/сети (с редакцией), локальные порты по конфигу — построены (VSK-BROWSER-B1/B2).
> Читать как отправную точку, не как текущее состояние.
