...
> **Исторический снимок.** Метрики ниже зафиксированы 2026-07-12 на версии 2.0.5 и не описывают текущую архитектуру. В частности, долг по монолиту `electron/ipc/tool-handlers/delegation.ts` закрыт в `ea50927e`: файл стал re-export, реализация разбита на модули в `delegation/`. Для текущего состояния использовать свежий запуск `node scripts/code-health-report.mjs` и верхний снимок в `STATUS.md`.
