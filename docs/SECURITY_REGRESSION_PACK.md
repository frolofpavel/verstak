...
| SEC-CMD-05 | command-exec | CWE-807 | block | `tests/security/connector-id-integrity.test.ts` | active |
| SEC-CMD-06 | command-exec | CWE-862 | block | `tests/security/browser-plan-mode.test.ts` | active |
| SEC-CMD-07 | command-exec | CWE-863 | block | `tests/security/browser-url-rules.test.ts` | active |
| SEC-CMD-08 | command-exec | CWE-863 | block | `tests/security/popup-navigation.test.ts` | active |
| SEC-CMD-09 | command-exec | CWE-693 | block | `tests/security/responsible-coverage.test.ts` | active |
...
- The connector gate and the connector executor read the identifier from one place (`canonicalConnectorId`), so a verdict can no longer be obtained for one connector while another is called. Two divergences were live: the gate read `args.connector ?? args.id` while the handler executed `args.id` (so `{id:'telegram', connector:'onec'}` was judged as harmless `onec` and delivered a Telegram message), and alias canonicalisation ran *after* the verdict, so a rule written against the canonical id was bypassed by writing the alias. Arguments naming two different connectors are refused outright rather than silently resolved in favour of one — a control mutation proves this refusal carries its own pin, because after the single-source fix the divergence alone is no longer exploitable. No key allowlist: connector parameters are open-ended and forwarded as-is, and the identifier was the only key a verdict is made from.
- Plan mode does not click in the browser. The mutating-browser category is a **list** (`MUTATING_BROWSER_TOOLS`) rather than a literal comparison, so a future `browser_type` is gated by default instead of by the next author's memory. Reading — `browser_read_page`, `browser_screenshot`, `browser_navigate` — still works in plan: the mode exists for reading, and banning it would have been a "fix" that removes the feature. The threshold for `ask`/`auto` is deliberately not decided in code: clicks now leave a trace, so the person picks it on real numbers.
- Browser rules judge the argument that will actually run, and a `confirm` verdict really stops the call. Both halves were broken: `extractArgText` did not know browser tools and returned an empty string, so a pattern rule — including an **absolute `deny`** — silently matched nothing; and the handler recognised only `block`, letting `confirm` fall through into execution, which would have made any future URL classifier falsely green. Known and pinned boundary: in rule patterns `*` does not cross `/`, so URLs need `**` — the glob semantics are shared with path rules and were left unchanged on purpose.
- A popup opened by the page (`target=_blank`, `window.open`) is judged by the same `applyPermissionRules` and under the same tool name as ordinary navigation, so one user rule closes both doors. Previously the handler checked the scheme only and redirected any http(s) URL into the webview silently — an ungated door right next to the gated one, reachable by clicking a `target=_blank` link. Declared, pinned boundary: `setWindowOpenHandler` is synchronous, so the agent mode does not apply here (a popup is a page event, not a tool call) and an `ask` rule means **refusal**, not a question — there is nowhere to ask from, and loading silently would violate the very rule the person wrote. Non-http(s) schemes never load.
- The responsible-action classifier normalises the command exactly as its denylist twin does (`deobfuscateShellWord`, reused rather than copied), so `"git" push` and `g^it push` no longer skip the pause — the regex approach had been taken from the denylist without its load-bearing premise. Coverage now includes global git flags (`git -C … push`, `--git-dir=`, `-c k=v`, `subtree push`), everyday publishing (`npm/yarn/pnpm run deploy`, `make deploy`, `docker push`, `helm upgrade`, `kubectl apply`, `gh pr/repo/gist create`, `aws s3 cp|sync`, `terraform apply`, `wrangler/firebase/flyctl deploy`), upload forms beyond `-X`/`-d` (`curl -T|--upload-file|-F|--data-binary`, `wget --post-data`), ssh/scp/rsync addressed by a `~/.ssh/config` alias rather than `user@host`, and `git clean -fd|--force`. Universal connectors are judged by operation, not by name alone: `ssh(run_remote)`, `http(POST|PUT|PATCH|DELETE)`, `gsheets(append_row|update_cell)` — reading through the same connectors stays silent. **Declared, pinned gaps:** home-grown wrappers (`./scripts/release.js`, `python upload.py`), indirection where the command words are absent from the string (`X=push; git $X`), and non-Latin forms are NOT caught, and the matcher works on substrings, so mentioning a command inside text costs one extra question. The honest answer for anything uncovered is a user rule, which works since SEC-CMD-07.
