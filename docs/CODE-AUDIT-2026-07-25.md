...
**Остаток B (часть 2) — нижняя строка композера.** Блок `gg-composer-hint` вместе с
`gg-composer-meta`, ~247 строк: подсказки стрима, счётчик токенов и ценник, Σ за
сессию, кнопка отката, автопрокрутка, `DevTaskBadge`, поповер «Инструменты чата»,
турбо-кнопка, вход в pipeline, «Прогоны», `ModePicker`/`ModelPicker`/`IntensityToggle`/
`PromptRouteControl`. Тянет за собой локальные `TokenPreviewMeter` и `formatTokens`
(используются только там) — их переносить вместе. Связей с состоянием чата ~30, но
тело JSX правки не требует: если пропсы назвать теми же именами, что нынешние
переменные замыкания, разметка переезжает дословно. Пины на поповер инструментов и
автопрокрутку в сетке уже есть.
