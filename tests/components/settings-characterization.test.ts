...
describe('Settings characterization — навигационная оболочка', () => {
  ...
  // Заголовок раздела (section-bar h2) уникально отражает активную вкладку — целимся в него,
  // а не в getByRole('heading') (у SubscriptionsTab свой h2 «Подписки» → два совпадения).
...
describe('Settings characterization — вкладка «Профили» (состояние до 2.0.8-G)', () => {
  it('отключена и помечена «Скоро»; клик не переключает раздел', async () => {
    mountSettings()
    await waitFor(() => expect(screen.getByText('Скоро')).toBeTruthy())
    const soonBtn = screen.getByText('Скоро').closest('button') as HTMLButtonElement
    expect(soonBtn.disabled).toBe(true)
    expect(soonBtn.getAttribute('aria-disabled')).toBe('true')
    expect(soonBtn.getAttribute('aria-selected')).toBe('false')
    fireEvent.click(soonBtn)                                   // disabled → onClick не срабатывает
    expect(soonBtn.getAttribute('aria-selected')).toBe('false')
  }, 10000)
})
