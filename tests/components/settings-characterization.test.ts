...
describe('Settings characterization — навигационная оболочка', () => {
  ...
  it('вкладка «Подписки» содержит управление: добавление аккаунта доступно из неё', async () => {
    mountSettings({ initialTab: 'subscriptions' })
    await waitFor(() => expect(sectionTitle()).toBe('Подписки'))
    await waitFor(() => expect(screen.getByText('+ Аккаунт')).toBeTruthy())
  }, 10000)
...
// 2.1.3-R1 БЛОКЕР 4: ошибки подписок НЕ проглатываются молча — видимый status/error
// вне формы добавления (она может быть закрыта в момент сбоя).
describe('Settings characterization — «Подписки»: видимые ошибки (2.1.3-R1)', () => {
  // Renderer-safe DTO config-dir аккаунта (как из subscription-accounts:list).
  const ACCT = {
    id: 1, providerId: 'codex-cli', label: 'Work', authMode: 'config-dir',
    state: 'login-required', active: true, lastUsedAt: null, hasCredential: false,
  }
  const mountWith = (subscriptionAccounts: Record<string, unknown>) => {
    mock = makeApiMock({ ...SETTINGS_DEFAULTS, subscriptionAccounts })
    vi.stubGlobal('window', Object.assign(globalThis.window, { api: mock.api }))
    return mountSettings({ initialTab: 'subscriptions' })
  }

  it('ошибка загрузки списка показывается как ошибка, а не как «аккаунтов нет»', async () => {
    mountWith({ list: async () => { throw new Error('db locked') } })
    await waitFor(() => expect(screen.getByText(/не удалось загрузить/i)).toBeTruthy())
    expect(screen.queryByText(/Пока нет подписочных аккаунтов/)).toBeNull()
  }, 10000)

  it('login fail → видимое сообщение (форма добавления закрыта)', async () => {
    mountWith({
      list: async () => [ACCT],
      login: async () => ({ ok: false, error: 'CLI не найден в PATH.' }),
    })
    await waitFor(() => expect(screen.getByText('Work')).toBeTruthy())
    fireEvent.click(screen.getByText('Войти'))
    await waitFor(() => expect(screen.getByText('CLI не найден в PATH.')).toBeTruthy())
  }, 10000)

  it('login ok → статическое сообщение про терминал, без внутреннего command/пути', async () => {
    mountWith({
      list: async () => [ACCT],
      login: async () => ({ ok: true, command: 'CODEX_HOME=C:\\secret\\path codex login' }),
    })
    await waitFor(() => expect(screen.getByText('Work')).toBeTruthy())
    fireEvent.click(screen.getByText('Войти'))
    await waitFor(() => expect(screen.getByText(/терминал/i)).toBeTruthy())
    expect(screen.queryByText(/secret\\\\path/)).toBeNull()
  }, 10000)

  it('Doctor fail → видимое сообщение даже при закрытой форме добавления', async () => {
    mountWith({
      list: async () => [ACCT],
      doctor: async () => ({ ok: false, error: 'Аккаунт не найден.' }),
    })
    await waitFor(() => expect(screen.getByText('Work')).toBeTruthy())
    fireEvent.click(screen.getByText('Проверить'))
    await waitFor(() => expect(screen.getByText('Аккаунт не найден.')).toBeTruthy())
  }, 10000)
})
