...
beforeEach(() => {
  ...
  useProject.setState({
    ...
    sendOwners: {}, chatSessions: [{ id: 7 }] as never, helpMode: false }, false); seedActive(useProject, { messages: [], isStreaming: false, sessionUsage: emptySessionUsage() })
})
...
describe('usage-событие → sessionUsage (Фаза 2.2)', () => {
  ...
  it('deprecated-мост cacheCreationInputTokens тоже доезжает (старые эмиттеры)', () => {
    ...
    expect(active(useProject.getState()).sessionUsage.cacheWriteTokens).toBe(25000)
  })
