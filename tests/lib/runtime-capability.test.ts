...
describe('runtimeCapability — честный уровень контроля из provider+transport', () => {
  ...
  // 2.0.4: режим Tunnel (Claude Code) — по рантайму как CLI (loop внешний), но с проекцией
  // таймлайна (observed), не full. Не «API».
  it('Tunnel-транспорт: capability как у наблюдаемого CLI, не full', () => {
    const cap = runtimeCapability('claude-cli', 'Tunnel')
    expect(cap.toolExecution).toBe(false)   // loop внутри claude, не наш
    expect(cap.tier).toBe('observed')       // claude-cli в CLI_WITH_TIMELINE
    expect(cap.toolVisibility).toBe(true)   // проекция таймлайна есть
  })
})
