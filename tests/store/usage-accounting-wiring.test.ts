 ...
 *
 * Цепочка: usage-событие (main) → Chat.tsx addUsage → active(store).sessionUsage.inputAccounting
...
describe('проводка inputAccounting до ценника чата (2.0.8-E хвост)', () => {
  beforeEach(() => {
    // 4.4: usage живёт в bundle активного чата — сбрасываем через chats.
    useProject.setState({ activeChatId: 1, chats: {} }, false)
  })
  ...
  it('addUsage сохраняет семантику провайдера в sessionUsage', () => {
    ...
    expect(active(useProject.getState()).sessionUsage.inputAccounting).toBe('exclusive')
  })
  ...
  it('дефект B в ценнике: Claude (exclusive) с большим кэшем считается по полному input', () => {
    ...
    const u = active(useProject.getState()).sessionUsage
