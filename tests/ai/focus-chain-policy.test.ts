...
// DEFAULT_AGENT_TURNS = 8: цикл идёт turn ∈ 0..7, и модуль 8 внутри бюджета
// недостижим по построению. Не «редко», а НИКОГДА. Плюс реинжект не возвращался
// после компакции — а состояние теряется там в первую очередь.
...
describe('shouldReinjectFocus — V2-1', () => {
  ...
  it('РЕПРО МЁРТВОСТИ: на дефолтном бюджете 8 ходов реинжект случается хотя бы раз', () => {
