...
// КОНТРАКТ ЭТОГО БЛОКА ОТМЕНЁН 11.08 (V2 ось A) — и снят тем же коммитом, что
// его отменил (§3.1). Пины ниже стерегли поведение «ждать карту, но не дольше
// бюджета». Замер показал, что этот компромисс платит и ничего не покупает: на
// каталоге в 44 964 файла «Привет» стоил 8461 мс, после которых пакет всё равно
// собирался БЕЗ карты. Ожидание убрано целиком — карта берётся готовой из кэша
// либо не берётся вовсе, а модель запрашивает её инструментом get_project_map.
// Новое поведение закреплено в tests/ai/context-pack-pull.test.ts.
//
// `withBudget` остаётся живым хелпером (его пины выше не тронуты) — он просто
// больше не применяется к картам в сборке контекста.
describe('V2 ось A: сборка контекста карту НЕ ЖДЁТ (прежний бюджет отменён)', () => {
  it('бюджетная константа сохранена как значение, но ожидания карт больше нет', () => {
    expect(MAP_BUDGET_MS).toBeGreaterThan(0)
  })

  it('карта прогрета → она в пакете', async () => {
  ...
  it('карты нет в кэше → пакет собран БЕЗ неё и говорит об этом ЯВНО', async () => {
    ...
    expect(pack, 'модели не сказали про инструмент, ради которого всё и делалось').toContain('get_project_map')
    expect(pack).not.toContain('project_map (compact)')
  })

  it('ГРАФ, которого нет в кэше, тоже оставляет след — он и стоит дорого', async () => {
    // Половина, на которую подозрение падает первой (карта проекта), упирается в
    // MAX_TOTAL_FILES = 2000 и стоит ~0.14 с. Всю цену платит граф зависимостей —
    // потолка у него нет, он читает и разбирает каждый кодовый файл дерева (на
    // каталоге приёмки — 26 288 файлов, порядка 80 с). Значит именно его выпадение
    // обязано быть видимым, иначе след стоит там, где почти не срабатывает.
    const pack = await buildContextPack({ projectPath: dir })

    expect(pack, 'граф выпал молча — модель не знает, почему исчезли связи').toContain('dependency_hubs:')
    expect(pack).toContain('граф зависимостей в этом запросе не приложен')
    expect(pack, 'модели не сказали, чем пользоваться вместо графа').toContain('search_project')
  })

  it('КОНТРОЛЬ: когда обе половины прогреты, следов об отсутствии в пакете нет', async () => {
    await getProjectMap(dir)
    await getDependencyMap(dir)
    const pack = await buildContextPack({ projectPath: dir })

    expect(pack).not.toContain('не приложена')
    expect(pack).not.toContain('не приложен')
  })
})
