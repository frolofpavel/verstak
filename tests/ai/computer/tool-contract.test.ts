...
describe('R2 Computer Use: production tool contract', () => {
  ...
  it('describes the production UIA-only boundary without promising disabled global input', () => {
    const descriptions = new Map(TOOL_DEFS.map(tool => [tool.name, tool.description]))
    expect(descriptions.get('computer_click')).toMatch(/UIA.*(?:Toggle|SelectionItem).*координат.*отключ/iu)
