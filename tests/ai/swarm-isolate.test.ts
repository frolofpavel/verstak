...
describe('swarm isolate — изоляция executor в worktree (T1.2 HIGH-фикс)', () => {
  ...
  it('isolate=true: запись executor НЕ в главном дереве, diff дошёл до арбитра', async () => {
    ...
    // 60s: тест спавнит реальный git worktree; под нагрузкой pre-commit (safe-rebuild
    // + параллельные воркеры) 30s флейчил по таймауту при зелёном изолированном прогоне.
  }, 60000)
