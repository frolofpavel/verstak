...
//
// ЧИТАТЬ ПЕРЕД ТЕМ, КАК ЧИНИТЬ КРАСНОТУ В ЭТОМ ФАЙЛЕ. Два пина ниже (блок «чего
// сканер на stderr НЕ ловит») фиксируют ИЗВЕСТНЫЕ пробелы словаря имён —
// опаковый секрет в имени пути и переменная окружения с неизвестным сканеру
// именем. Они утверждают, что эти формы проходят НАСКВОЗЬ. Если сканер однажды
// научится их узнавать, пины покраснеют — и это НЕ регрессия и НЕ повод
// подогнать утверждение под новый вывод: это сигнал переписать границу целиком,
// вместе с отчётом о том, что теперь считается очищенным.
