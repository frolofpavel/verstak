...
describe('checkpoint-throttle — обуздание write-amplification чекпойнтов (1.9.7 #7)', () => {
  ...
  it('РЕВЬЮ: крупная-но-легитимная история 1M-модели (>4МБ) СОХРАНЯЕТСЯ (не режем resume)', () => {
    // 8МБ несжатой истории (типично для 1M-context до auto-compact). Старый cap
    // 4МБ рубил бы это → resume без контекста. Дефолтный cap теперь 32МБ.
    const big = 'x'.repeat(8 * 1024 * 1024)
    const d = decideCheckpointSave(5, big, undefined)
    expect(d.save).toBe(true)
    expect(d.reason).toBe('first')
  })
