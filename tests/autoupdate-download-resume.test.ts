...
// РАЗМЕРЫ ЗДЕСЬ НЕ ПРОИЗВОЛЬНЫ — не уменьшать, см. ниже.
// `download.ts` пишет в `.part` через createWriteStream с highWaterMark 2 МБ (и такой же
// буфер у Transform-крана). Пока принято меньше буфера, на диск не уходит НИЧЕГО, а обрыв
// рвёт поток с ещё не сброшенным буфером — `.part` остаётся нулевым, продолжать нечего, и
// следующая попытка ЗАКОННО идёт без Range. Прежняя фикстура (300 КБ, обрыв на 100 КБ) была
// целиком ВНУТРИ буфера, поэтому «докачка просит Range» проверялась на условии, которого в
// ней не возникало: тест зеленел, только когда поток случайно успевал сбросить часть, и
// падал примерно в четверти прогонов (09.08, поймано pre-commit хуком на релизе 2.4.7).
// Утверждения тестов НЕ менялись — изменён только вход, чтобы они проверяли то, что заявляют.
const DATA = Buffer.alloc(10_000_000) // 10 МБ — заведомо больше буферов (2 МБ + 2 МБ)
...
describe('автообновление: докачка установщика (P0 — доставка релизов)', () => {
  it('обрыв посреди загрузки → файл ВСЁ РАВНО докачивается, sha512 сходится', async () => {
    killAfterBytes = 6_000_000 // умрём на 60%, ЗА пределами буферов записи (2+2 МБ) — часть реально на диске
  ...
  it('после обрыва докачка идёт с Range, а НЕ с нуля (иначе 360 МБ не доедут никогда)', async () => {
    killAfterBytes = 6_000_000 // за пределами буферов записи — см. шапку про размеры
