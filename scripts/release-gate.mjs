...
// 09.08, СЛИЯНИЕ линии native-ABI+smoke (эта ветка) поверх main 4907: три fail-closed
// точки класса «native ABI в релизе» (обеззараженная самопочинка / гард afterPack / ABI
// на выходе сборки) + install smoke-тест (доказан на ИЗГОТОВЛЕННОМ мёртвом артефакте:
// целая→PASS, без .node→FAIL, без preload→FAIL через render-краш). +20 статичных пинов:
// native-abi 5, repair 3, afterPack 4, smoke-verdict 8. ИЗМЕРЕНО на слитом дереве: 4927 /
// 0 падений (ожидание 4907+20=4927; provider-model-drift мои файлы не задевает — все статичны).
const EXPECTED_TOTAL_TESTS = 4927
