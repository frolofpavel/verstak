...
// Та же оговорка: замер ОДНОЙ линии; при вливании нескольких — перемерить на слитом.
// 15.08 хвост той же линии → 5541 ИЗМЕРЕНО: +8 пинов (§2.5 web-политика доменов доезжает
// до запроса, §2.8 запрет секрето-путей у edit_spreadsheet).
// Та же оговорка: замер ОДНОЙ линии; при вливании нескольких — перемерить на слитом.
// 15.08 «пустой текст консоли встроенного браузера» → 5519 ИЗМЕРЕНО на ветке линии
// (`npm run test:fast -- --maxWorkers=4`, 0 падений, 641 файл). Прирост +8 — сетка
// producer↔читатель для формы записи консоли (tests/contracts/browser-console-record-shape).
// Та же оговорка: замер ОДНОЙ линии; при вливании нескольких — перемерить на слитом.
// 15.08 слито ДВЕ линии (дыры ревизии + консоль браузера) — число ИЗМЕРЕНО
// на слитом дереве, а не сложено из приростов линий (§3.1).
// 15.08 P3 кусок 2 (токен-бенчмарк браузера, понижение VSK_SNAPSHOT_TOP_N 150→50)
// → 5464 ИЗМЕРЕНО на ветке линии (`npm run test:fast -- --maxWorkers=4`, 0 падений):
// +1 пин «find видит цель ЗА пределами top-N» — он держит обоснование самого порога
// (цель за картой не потеряна, её достаёт find), проверен мутацией. Та же оговорка:
// замер ОДНОЙ линии, при вливании нескольких — перемерить на слитом дереве.
// 15.08 УБОРКА §1.1 ревизии: удалён мёртвый экран AgentRunInspector вместе с
...
// 15.08 УБОРКА §3.1 ревизии: полярность рантайм-флагов сведена в один источник
// (shared/contracts/runtime-flag-policy), анти-дрейф-страж переработан под новый
// контракт: +3 пина (дубль не вернулся во ВСЁМ electron/ · контрольный кейс к нему ·
// таблица renderer не объявляет полярность заново). → 5546 ИЗМЕРЕНО
// (`npm run test:fast -- --maxWorkers=4`, 0 падений, 641 файл).
// 15.08 УБОРКА §3.2 ревизии: список секрето-путей сведён в один источник
// (secret-scanner.secretPathDenyGlobs), рукописная вторая редакция в claude-cli
// удалена. Новая сетка tests/security/secret-path-single-source.test.ts: +50 пинов
// (контрольная пара «секрет закрыт на обоих путях · обычный файл читается на обоих»
// по корпусу из 34 секретных и 9 обычных путей, свод расхождений, анти-дрейф по
// всему electron/ с контрольным кейсом). → 5596 ИЗМЕРЕНО
// (`npm run test:fast -- --maxWorkers=4`, 0 падений, 642 файла).
// → 5601 ИЗМЕРЕНО 16.08 (враждебное ревью §1: отказ установки поверх ЗАПУЩЕННОГО
// приложения + откат, который восстанавливает прежние файлы вместо удаления —
// tests/installer/install-guard.test.ts, 5 тестов). `--maxWorkers=4`, 0 падений,
// 643 файла. → 5608 ИЗМЕРЕНО 16.08 (§2: подхват ключа из окружения выключен по
// умолчанию, живёт за VERSTAK_ALLOW_ENV_KEYS и виден в интерфейсе —
// tests/security/env-secrets.test.ts, 7 тестов). 0 падений, 644 файла.
// → 5624 ИЗМЕРЕНО 16.08 (первые пять минут §3/§4/§5: гард первого запуска,
// склейка мягких переносов в релиз-нотах, проект не назначается на домашний
// каталог, один язык на главном экране и в онбординге). 0 падений, 646 файлов.
// → 5626 ИЗМЕРЕНО 16.08 (§6: счётчики витрины убраны из README и онбординга —
// tests/docs/showcase-counters.test.ts). 0 падений, 647 файлов.
// → 5627 ИЗМЕРЕНО 16.08 после ЖИВОЙ приёмки: она поймала, что гард первого
// запуска по «онбординг пройден» не срабатывает (онбординг завершается в той же
// сессии раньше показа модалки) — признаком стала метка рождения профиля.
// → 5639 ИЗМЕРЕНО 16.08 (цель 2.7.0, шаг 2 пункты 2–3: мультиагент снят с
// витрины, панель «Агенты» разгружена, вход в неё стал условным). 0 падений,
// 650 файлов, --maxWorkers=4. Прирост по факту +12 при 11 добавленных `it` —
// ровно тот случай, ради которого эталон ИЗМЕРЯЕТСЯ, а не складывается из
// приростов (CLAUDE.md §3.1): часть тестов рождается динамически.
// → 5651 ИЗМЕРЕНО 16.08 (цель 2.7.0, шаг 2: completion gate поднят до уровня
// verify-гейта снятой кнопки «До результата» — засчитывается ИСХОД проверки, а не
// её факт). 0 падений, 650 файлов, --maxWorkers=4. Прирост +12 при 12 добавленных
// `it` (9 чистых + 3 рантаймовых).
// → 5652 ИЗМЕРЕНО 16.08 (тот же шаг 2: кнопка «До результата» и «Прогоны» сняты с
// экрана). Прирост +1: два пина, стерёгшие ОТМЕНЁННЫЙ контракт «у доведения есть
// свой вход», сняты, три встали взамен — включая контрольный «экран живой».
// → 5653 ИЗМЕРЕНО 16.08 (тот же шаг 2, пункт 4: скрепка и папка сведены в одну
// кнопку). Прирост +1: пин контракта A1 «📁 стоит в композере» снят как
// отменённый, взамен два — «файловая кнопка одна» и «действие не потеряно».
// → 5657 ИЗМЕРЕНО 16.08 (ШАГ 3 цели 2.7.0: панель над композером схлопнута; на
...
// -> 5848 ИЗМЕРЕНО 06.09 (реестр возможностей, шаг 1). Прирост +44: контракт
// паспорта, проекция четырёх источников, оверлей доверия в БД и служба.
// Из них шесть контрольных: без них пины «доверие не переносится» и «фильтр
// не пропускает выключенное» зелены и у реализаций, которые не делают ничего.
// -> 5870 ИЗМЕРЕНО 06.09 (губернатор доверия, шаг 3). Прирост +22: расчёт
// уровня по фактам и врезка в resolveDecision. Ключевые из них — переборные:
// слой доверия не имеет права ослабить решение НИ ПРИ КАКОЙ комбинации
// уровня, режима и вызова, и пауза перед ответственным действием не
// становится мягче ни на одном уровне. Дыра в таком слое не краснеет сама.
// -> 5879 ИЗМЕРЕНО 06.09: панель реестра под jsdom. Прирост +9. Живая приёмка
// доказала сборку паспорта в главном процессе и мост наружу (316 возможностей,
// поля непустые, ошибок нет), но до ОТРИСОВКИ не дошла: экран «Скиллы» требует
// открытого проекта, а выбор папки идёт через системный диалог. Эти пины
// закрывают вторую половину — и проверены мутацией, а не написаны для счёта.
// -> 5893 ИЗМЕРЕНО 07.09 (происхождение прогона, шаг 4). Прирост +14: связь
// прогона с возможностями и сборщик доказательств. Главный из них — что после
// подмены содержимого прежние прогоны в доказательства НЕ идут: доверие
// принадлежит проверенному содержимому, иначе правило «обновление не повышает
// доверие» обходилось бы редактором.
// -> 5900 ИЗМЕРЕНО 07.09 (губернатор доходит до каждого гейта). Прирост +7:
// пин обхода ИСХОДНИКОВ — обработчик, забывший передать уровень, не падает и
// не краснеет, он просто работает по старым правилам. Плюс путь MCP: он идёт
// МИМО resolveDecision и без отдельной строки остался бы единственной дорогой
// к инструменту в обход слоя доверия.
// -> 5937 ИЗМЕРЕНО 08.09 (постоянные задачи, шаги 5-6). Прирост +37: шина
// событий, решение о пробуждении, durable хранилище и цикл целиком. Главный —
// задача, застрявшая в «выполняется» после падения, уходит на ПАУЗУ, а не в
// активные: неизвестно, успело ли пробуждение выполнить своё действие, и
// повторить платёж или отправку дороже, чем спросить человека.
// -> 5949 ИЗМЕРЕНО 08.09: постоянные задачи получили выход наружу. Прирост +12:
// предохранители создания проверяются напрямую чистой функцией, а не через мок
// ipcMain, плюс пин на дефект, пойманный соседним пином при врезке — секция
// звала мост без проверки и роняла ВЕСЬ экран расписания там, где моста нет.
// -> 5972 ИЗМЕРЕНО 08.09 (Workflow Scientist, шаг 7). Прирост +23: детектор
// дорогого/проваленного workflow, гипотеза с ОДНИМ изменённым фактором,
// сравнение и артефакт итога. Главные два: дешевизна не перевешивает
// качество (кандидат, который стал чаще ошибаться, не продвигается), и
// запрет самовольного применения закреплён в СХЕМЕ, а не в коде.
// -> 5992 ИЗМЕРЕНО 09.09: обновление открытых карточек постоянных задач,
// очистка таймера, защита от позднего ответа другого проекта и медленного IPC.
// -> 6005 ИЗМЕРЕНО 10.09: первый запуск с IRI Gateway, проверка ключа до
...
// -> 7459 ИЗМЕРЕНО 19.09: live-path калькулятора (gateway 5000-char slice
// отрезал observationText) — 0 падений, 20 skip, --maxWorkers=4.
// -> 7466 ИЗМЕРЕНО 19.09: ранний стоп Computer Use показывается человеку
// текстом, а не машинным кодом — 0 падений, 20 skip, --maxWorkers=4.
// -> 7468 ИЗМЕРЕНО 19.09: детектор зацикливания различает клики по разным
// элементам (вырожденная подпись глушила третий клик) — 0 падений, 20 skip,
// --maxWorkers=4.
// -> 7474 ИЗМЕРЕНО 19.09: незакрытая отметка Computer Use больше не запирает
...
// -> 7487 ИЗМЕРЕНО 19.09: правый клик в поле ввода даёт «Вставить» (ключ
// Gateway нельзя было вставить мышью) — 0 падений, 20 skip, --maxWorkers=4.
const EXPECTED_TOTAL_TESTS = 7487
