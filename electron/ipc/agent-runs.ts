...
export function registerAgentRunsIpc(
  ...
  ipcMain.handle('agent-runs:get', (_e, runId: string) => {
    ...
    // Вторая ветка роадмапа — «незакрытая review-сессия» — СНЯТА как требование
    // 27.07 (разбор в docs/CODE-AUDIT-2026-07-25.md). Коротко: недостающее — не
    // код, а данные. `chat_sessions(kind='review')` не несёт признака
    // done/closed, и завести его = построить lifecycle ревью, которого в планах
    // нет. Доступный сигнал «review-прогон сейчас идёт» значит ДРУГОЕ: это работа
    // в процессе, а не «ждёт человека», и подстановка его сюда заставила бы
    // статус врать. Реальную потребность закрывает ветка выше: verify=fail на
    // завершённом прогоне. Вернуть, если появится lifecycle ревью с закрытием.
    if (run.status === 'done') {
