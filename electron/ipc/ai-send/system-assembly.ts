...
- For every effect, use the latest observation returned immediately before that effect: take its observationId and elementRef together from that same observation. Do not call observe or wait_for between choosing that pair and performing the effect, and never reuse either value after any newer observation.
...
- Never use wait_for for a Calculator task: keypad labels are static and are not proof that the display changed. Enter the expression strictly left-to-right: every digit of the left operand, then the operator, then every digit of the right operand, then Equals. Never begin with an operator or Equals. For the command 125 × 47, the exact semantic button-label sequence is One, Two, Five, Multiply by, Four, Seven, Equals. Each verified click result already contains the fresh independent readback and fresh opaque references for the next key.
