...
// ДЕКЛАРАЦИЯ БЕЗ ПОТРЕБИТЕЛЯ — названо прямо (§1.4 ревизии, 15.08). Список
// префиксов не применяет НИКТО: ни фильтр окружения агента, ни редакция логов;
// его читает только tests/security/dotenv-guard.test.ts, где утверждение звучит
// как «keeps … env prefixes out of agent context» — то есть тест обещает
// защиту, которой в коде нет (позиция §2.10 ревизии, её закрывает линия тестов).
// Оставлен намеренно: это не мусор, а не доведённая до конца защита, и удалить
// список — значит потерять и сам замысел, и след о нём. Кто будет строить
// фильтр env — начинает отсюда. Обход MCP-процессов живёт своим allowlist'ом
// (electron/mcp/client.ts, MCP_ENV_ALLOWLIST) и этот список не использует.
...
// см. tool-handlers/file-ops.ts). Наружу из main это содержимое не выходит
// никогда: маска подставляется в САМО событие подтверждения, а не перед
// отрисовкой. Причина доказана трассировкой, а не осторожностью — событие
// прогона, запущенного с телефона, форвардится целиком на внешний relay по
// HTTP, так что маска в renderer оставила бы секрет уезжающим с машины.
//
