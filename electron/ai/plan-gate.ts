...
export function resolvePlanGate(decision: PlanDecision, feedback: string | undefined, title: string): PlanGateOutcome {
  ...
  switch (decision) {
    ...
    case 'revise':
      // Инструмент здесь НЕ называется сознательно (доработка после ревью
      // 28.07): текст «обнови через create_plan» противоречил тому, что тут же
      // дописывает plan-await («правь через replan_plan, новый не создавай»).
      // Одна инструкция — одно указание; имя инструмента даёт тот, кто знает
      // путь, по которому пойдёт продолжение.
      return {
        result: `✏ Пользователь просит ДОРАБОТАТЬ план «${title}»${fb}. Учти замечания и снова предложи план на одобрение. НЕ начинай выполнение.`,
        newMode: null,
      }
    case 'reject':
      return {
