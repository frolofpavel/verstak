 ...
 * память» и «отправил модели» проходило от 7 м 20 с до 14 м 52 с (зависли ШЕСТЬ
 * отправок, освободились двумя пачками по три), вызовов инструментов в это время
 * не шло вовсе. Проект тех прогонов — каталог на 44 949 файлов.
 *
 * ЧТО ИМЕННО ТОРМОЗИЛО — измерено, а не выведено из правдоподобия, и это НЕ та
 * половина, на которую падает подозрение первой. `buildProjectMap` упирается в
 * `MAX_TOTAL_FILES` = 2000 и на том же каталоге стоит ~0.14 с. Всю цену платит
 * `buildDependencyMap`, у которого потолка нет вовсе: он читает и разбирает
 * регулярками КАЖДЫЙ кодовый файл дерева (на том каталоге — 26 288 файлов,
 * 252 МБ, порядка 80 с на одну сборку при тёплом кэше ОС). Отсюда и бюджет
 * общий: дешёвая половина в него никогда не упрётся, дорогая упрётся всегда.
 *
 ...
 */
// НЕ ПРИМЕНЯЕТСЯ С 11.08 — и это решение, а не недосмотр (§1.4 ревизии, 15.08
// фиксирует константу и withBudget как живущие своим тестом). Ожидание карты
// снято целиком в V2 ось A; константа и хелпер оставлены тогда же осознанно —
// см. комментарий в месте, где раньше стоял вызов, и tests/ai/context-pack-budget.
...
export async function buildContextPack(input: ContextPackInput): Promise<string> {
  ...
  //
  // Д1: тот же бюджет, но именно ЗДЕСЬ он и срабатывает. В отличие от карты
  // проекта у графа нет потолка файлов: он читает и разбирает каждый кодовый
  // файл дерева, поэтому на большом каталоге просрочивается всегда, а карта —
  // почти никогда. След обязателен по той же причине, что и у карты: молча
  // выпавший граф неотличим от проекта без связей, и модель перестаёт видеть
