...
// ── Д7 (приёмка 10.08): гейт «цель закрыта?» на продлении бюджета ───────────
//
// Правило прогресса продлевает бюджет на любой новый факт и само по себе не
// знает, достигнута ли цель, — прогон, уже выдавший финальный ответ, молча ехал
// до потолка. Признак «цель закрыта» кодом не выводится (код видит меньше
// модели): первый ход каждого продления несёт эту ноту, и решение читается
// структурно — ответ без вызова инструментов завершает прогон, вызовы означают
// осознанное продолжение, о котором рантайм сообщает в ленту.
export function buildGoalCheckNote(): string {
  return [
    'Бюджет ходов на исходе. Прежде чем продолжать — реши, закрыта ли цель задачи.',
    'Если финальный ответ пользователю уже выдан и цель достигнута — остановись сейчас: ответь коротким итогом без вызова инструментов. Дополнительные улучшения предложи пользователю вопросом, а не молчаливой работой.',
    'Если цель НЕ закрыта — в том числе если ты нашёл ошибку в собственном результате — продолжай: первой строкой назови, что именно осталось, и работай дальше.',
  ].join('\n')
}
