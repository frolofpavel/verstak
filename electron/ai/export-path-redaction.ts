...
export function redactPathsForExport(text: string, ctx: PathRedactionContext): string {
  ...
  // структурному маркеру (два ведущих backslash), остальной путь/текст сохраняем. Хост —
  // до разделителя, пробела или конца строки: ловим и `\\хост\шара`, и голый `\\хост`
  // (в конце строки / перед пробелом — «ping \\FILESRV01»), который прежний вариант с
  // обязательным разделителем пропускал (ре-ревью C #3).
  //
  // ГРАНИЦА: forward-slash UNC (`//хост`) НЕ трогаем — слепая замена `//` сломала бы
  // `https://хост` (схема URL). Нативная Windows-форма UNC — backslash; forward-форму
  // редко встретишь в тексте, а порча URL хуже редкой утечки (ре-ревью C #4/#5, severity low).
  out = out.replace(/\\\\[^\\/\s]+/g, '\\\\[скрыто]')
