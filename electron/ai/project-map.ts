...
// Раньше дедупликация была только у фонового прогрева (warmProjectMaps), а сами
// get*-функции её не имели — параллельные отправки в один проект запускали
// независимые сборки. На каталоге из 44 949 файлов (C:\Users\Pavel\Downloads в
// тех прогонах) шесть отправок простояли от 7 до 15 минут и освободились двумя
// пачками по три, когда сборки наконец закончились.
//
// ЦЕНУ ПЛАТИТ ГРАФ, А НЕ КАРТА — записано после измерения, потому что подозрение
// естественно падает на карту, и следующий читатель иначе пойдёт оптимизировать
// не ту функцию. buildProjectMap упирается в MAX_TOTAL_FILES = 2000 и на том же
// каталоге стоит ~0.14 с. buildDependencyMap потолка не имеет вовсе: читает и
// разбирает регулярками каждый кодовый файл дерева (там — 26 288 файлов, 252 МБ,
// порядка 80 с на сборку). Дедупликация нужна обеим, но спасает она граф.
...
/**
 * V2 ось A (волна 2.6.0): карта БЕЗ ОЖИДАНИЯ — «есть готовая, отдай; нет — не держи».
 *
 * Прежний путь `withBudget(getProjectMap(...))` всё равно ЖДАЛ (до 8 с), и на
 * большом каталоге это ожидание не приносило ничего: замер «до» на 44 964 файлах
 * дал 8.5 с на «Привет», после которых пакет собирался БЕЗ карты — бюджет истекал.
 * Человек платил секунды за пустоту.
 *
 * Здесь ленивость, а НЕ классификатор «простой запрос» (§3.1: код в момент решения
 * видит меньше модели). Мы не угадываем, нужна ли карта — мы не платим за то, что
 * ещё не готово: `get_project_map` существует инструментом, и модель возьмёт карту
 * сама, когда она ей понадобится.
 *
 * Просроченный по TTL кэш отдаётся НЕМЕДЛЕННО (stale-while-revalidate), а обновление
 * уходит в фон. Отсюда «повторная отправка не пересобирает карту»: раньше через
 * 30 секунд TTL истекал и следующая отправка ждала полный обход заново.
 */
export function getProjectMapIfReady(root: string): ProjectMap | null {
  const cached = cache.get(root)
  if (cached) {
    // Просрочен → отдаём как есть и обновляем в фоне: устаревшая карта полезнее
    // паузы, а свежая приедет к следующей отправке.
    if (Date.now() - cached.timestamp >= CACHE_TTL) void getProjectMap(root, true).catch(() => null)
    return cached.map
  }
  // Кэша нет — запускаем сборку в фон и НЕ ждём. Дедупликация внутри getProjectMap
  // не даст запустить второй обход того же root.
  void getProjectMap(root, false).catch(() => null)
  return null
}

/** То же для графа зависимостей. Именно он платит цену на большом дереве (Д1):
 *  потолка файлов у него нет, на каталоге приёмки — порядка 80 с на сборку. */
export function getDependencyMapIfReady(root: string): DependencyMap | null {
  const cached = depCache.get(root)
  if (cached) return cached
  void getDependencyMap(root, false).catch(() => null)
  return null
}
