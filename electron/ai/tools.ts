...
Возвращает количество применённых блоков. Требует подтверждения пользователя как обычный write — diff отображается в том же модальном окне.`,
  ...
  },
  {
    ...
    description: 'Открыть URL во встроенном браузере Verstak. Возвращает финальный URL после редиректов. Вкладку «Браузер» открывать НЕ нужно — инструмент работает и без неё; если человек откроет вкладку, увидит ту же страницу, которой управляешь ты.',
    parameters: {
      type: 'object',
