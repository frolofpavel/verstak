...
Возвращает количество применённых блоков. Требует подтверждения пользователя как обычный write — diff отображается в том же модальном окне.`,
  ...
  },
  {
    ...
    description: 'Структурный снимок текущей страницы: список ИНТЕРАКТИВНЫХ элементов (ссылки, кнопки, поля) с НОМЕРАМИ, ролью и подписью. Дальше кликай по номеру через browser_click_by_number — не угадывай CSS-селектор. Номера действительны только до навигации: после перехода сделай снимок заново. Требует открытую вкладку Browser.',
    parameters: { type: 'object', properties: {} }
  },
  {
