...
Возвращает количество применённых блоков. Требует подтверждения пользователя как обычный write — diff отображается в том же модальном окне.`,
  ...
  },
  {
    ...
    description: 'Кликнуть по элементу из ПОСЛЕДНЕГО browser_snapshot по его номеру. Надёжнее browser_click: не надо угадывать селектор. Если номер устарел (была навигация или страница изменилась) — вернётся честная ошибка, сделай browser_snapshot заново. Требует открытую вкладку Browser.',
    parameters: {
      type: 'object',
      properties: {
