...
import { getProjectMap, invalidateProjectMap, markFileDirty, projectMapToText, invalidateDependencyMap } from './project-map'
import { resolveReadOnlyPath, resolveWritablePath } from './path-policy'
...
// P3 кусок 3: список сред и правило выбора — тоже один источник (см. там же).
import { BROWSER_ENVS, BROWSER_ENV_RULE } from '../../shared/browser-env'
...
Возвращает количество применённых блоков. Требует подтверждения пользователя как обычный write — diff отображается в том же модальном окне.`,
  ...
  },
  {
    ...
    description: 'Выполнить запрос к внешнему коннектору. id — id коннектора (НЕ имя tool). Для Wordstat сразу вызывай id="yandex_wordstat", op="get_top_requests" (phrase, regions?, num_phrases?) или op="get_wordstat" (phrases[]). list_connectors опционален: не пиши «Wordstat недоступен» без live-ответа connector_query. Для 1С (id="onec") — entity + filter/select/top или metadata:true. Для HTTP (id="http") — endpoint + method + path + query/body/headers. Креды и base URL берутся из настроек — НЕ передавай пароли в args.',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    // P3 кусок 3: точка входа выбирает СРЕДУ. Правило выбора не переписывается здесь
    // словами, а подставляется из BROWSER_ENV_RULE — копия текста разъехалась бы с
    // поведением и однажды соврала бы модели (§3.1).
    description: 'Открыть URL в браузере Verstak. Возвращает финальный URL после редиректов и поле mode — среду, в которой ты сейчас работаешь. Вкладку «Браузер» открывать НЕ нужно — инструмент работает и без неё; если человек откроет вкладку, увидит ту же страницу, которой управляешь ты. ' + BROWSER_ENV_RULE + ' Все остальные browser_* работают в той же среде, что и последний navigate — отдельных инструментов для второй среды нет. Закрыть чистую сессию досрочно: browser_close_session (иначе она закроется сама вместе с задачей).',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Закрыть чистую изолированную сессию браузера досрочно и вернуться во встроенный браузер. Нужен, только если сценарий в чистой сессии закончен, а задача продолжается: сама по себе сессия закрывается вместе с задачей в любом случае, в том числе при остановке. Если чистой сессии не было — честно ответит closed=false, ошибкой это не является.',
    parameters: { type: 'object', properties: {} }
  },
  {
  ...
  },
  {
    ...
    description: 'Структурный снимок текущей страницы: список ИНТЕРАКТИВНЫХ элементов (ссылки, кнопки, поля) с НОМЕРАМИ, ролью и подписью. Это ЗАПАСНОЙ путь — когда нужна карта ВСЕЙ страницы; для одной цели дешевле browser_find. На больших страницах список обрезается (truncated=true, поле count — сколько всего): недостающее ищи через browser_find. Дальше кликай по номеру через browser_click_by_number. Номера действительны только до навигации: после перехода сделай снимок заново.',
    parameters: { type: 'object', properties: {} }
  },
  {
    name: 'browser_find',
    description: 'ОСНОВНОЙ способ адресоваться к элементу: ищет по подписи/роли/тексту (например «отправить», «поле email», «link») и возвращает подходящие элементы с НОМЕРАМИ. Дальше кликай browser_click_by_number или вводи browser_type_by_number по номеру. Используй find ПЕРВЫМ — он дешевле browser_snapshot (отдаёт только нужное, а не всю страницу); полный снимок бери только когда нужна карта целиком. Ничего не нашлось — честный пустой ответ с подсказкой сделать снимок. Номера действительны до навигации.',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Что искать: видимая подпись, роль (link/button/textbox) или часть текста элемента.' },
        limit: { type: 'number', description: 'Сколько совпадений вернуть (по умолчанию 30, потолок 100).' }
      },
      required: ['query']
    }
  },
  {
    name: 'browser_click_by_number',
    description: 'Кликнуть по элементу из ПОСЛЕДНЕГО browser_snapshot по его номеру. Надёжнее browser_click: не надо угадывать селектор. Если номер устарел (была навигация или страница изменилась) — вернётся честная ошибка, сделай browser_snapshot заново.',
    parameters: {
      type: 'object',
      properties: {
        n: { type: 'number', description: 'Номер элемента из последнего browser_snapshot.' }
      },
      required: ['n']
    }
  },
  {
    ...
    description: 'Ввести текст в поле (input/textarea/contenteditable) из ПОСЛЕДНЕГО browser_snapshot по его номеру — для заполнения форм. Если номер устарел или элемент не текстовое поле — честная ошибка.',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Дождаться появления элемента на странице (по CSS-селектору ИЛИ видимому тексту) с таймаутом — после навигации/клика, пока страница догружается. Появился → ok. Не появился за отведённое время → честная ошибка (не тихая пауза).',
    parameters: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Прочитать ОШИБКИ и ПРЕДУПРЕЖДЕНИЯ из консоли текущей страницы (для отладки: JS-ошибки, упавшие ресурсы). Возвращает ОГРАНИЧЕННЫЙ список свежих сообщений (не весь лог), поле count — сколько всего. Секреты в тексте замаскированы. Пусто → на странице ошибок нет.',
    parameters: {
      type: 'object',
      properties: { limit: { type: 'number', description: 'Сколько последних ошибок/предупреждений вернуть (по умолчанию 20).' } }
    }
  },
  {
    name: 'browser_network',
    description: 'Прочитать сетевые запросы текущей страницы (fetch/XHR: метод, URL, статус, длительность) — для отладки API-вызовов. Заголовки авторизации и токены в URL замаскированы, тела запросов НЕ возвращаются (приватность). Ограниченный список свежих запросов, count — сколько всего.',
    parameters: {
      type: 'object',
      properties: { limit: { type: 'number', description: 'Сколько последних запросов вернуть (по умолчанию 30).' } }
    }
  },
  {
    name: 'web_search',
  ...
  {
    name: 'create_plan',
    description: 'Создать структурированный план многошаговой задачи (от 3 шагов). Информационный план НЕ останавливает работу; на согласование выносится только крупная (>5 шагов / >3 файлов / архитектура) или изменяющая задача. План отобразится во вкладке Plan; пользователь сможет выполнять шаги по одному.',
    parameters: {
      type: 'object',
      properties: {
  ...
  {
    // Задача 10 (оркестратор). Предлагается модели ТОЛЬКО при включённом флаге
    // orchestrator_default (гейт в runner-api). Триггер — решение МОДЕЛИ по ходу.
    name: 'spawn_task_session',
    description: 'Вынести отдельную задачу в СВОЮ ВИДИМУЮ сессию (отдельный чат), а не выполнять её в текущем разговоре. Используй, когда по ходу диалога возникает самостоятельная задача, которую логично вести отдельно: текущий чат остаётся местом мышления/проектирования, а задача уезжает работать в свою сессию и вернёт результат сюда. Это НЕ delegate_task (скрытый суб-агент внутри этого прогона) — это полноценная видимая дочерняя сессия со своей историей, которую человек может открыть. Человек видит карточку «задача ушла в сессию №N». Для узкой подзадачи с возвратом как tool_result используй delegate_task; для самостоятельной задачи, которую человек захочет видеть и вести отдельно, — этот инструмент.',
    parameters: {
      type: 'object',
      properties: {
        title: {
          type: 'string',
          description: 'Короткий заголовок задачи (для имени дочерней сессии и карточки-следа). Напр. «Собрать отчёт по Директу».'
        },
        task: {
          type: 'string',
          description: 'Полная самодостаточная формулировка задачи — станет первым сообщением дочерней сессии. Пиши так, чтобы сессия поняла задачу без контекста этого разговора.'
        }
      },
      required: ['title', 'task']
    }
  ...
  {
    name: 'draft_project_rules',
    description: 'P2: собрать ЧЕРНОВИК файла правил этого проекта (стек, команды тестов/сборки/линта, структура, стиль, запреты) по реальным файлам — package.json, конфигам, структуре каталогов. Ничего не выдумывает: чего в проекте нет, так и записывает («тестов не найдено»). Черновик показывается человеку обычным подтверждением записи. Если файл правил уже есть, он НЕ перезаписывается — черновик уходит отдельным предложением (.verstak/RULES.draft.md), если только не передан overwrite:true.',
    parameters: {
      type: 'object',
      properties: {
        overwrite: { type: 'boolean', description: 'Писать прямо в существующий файл правил вместо отдельного черновика. По умолчанию false.' }
      }
    }
  },
  {
    name: 'mutation_check',
    description: 'C2 (P6): доказать, что тест НЕ декоративный, мутацией фикса. Рантайм прогоняет указанный тест в ИЗОЛИРОВАННОМ git worktree на HEAD (мир без твоего незакоммиченного фикса; рабочее дерево не трогается): тест обязан ПОКРАСНЕТЬ. Зелёный на HEAD → тест декоративный, вердикт отклоняется. ГРАНИЦА: узкий прогон одного файла — полный набор этим не измерен; фикс обязан быть незакоммиченным. Бюджет по умолчанию 120 с (timeout_ms ≤ 300000); таймаут — не вердикт. Выключатель: настройка mutation_check_enabled.',
    parameters: {
      type: 'object',
      properties: {
        test_file: { type: 'string', description: 'Относительный путь теста, который должен покраснеть без фикса.' },
        timeout_ms: { type: 'number', description: 'Бюджет прогона в мс (по умолчанию 120000, максимум 300000).' }
      },
      required: ['test_file']
    }
  },
  ...
  // ─── Чек-лист проекта (§9 блока C). НЕ путать с todo_* выше: todo — оркестрация
  // внутри прогона, чек-лист — пользовательская сущность проекта, переживает
  // перезапуск и закрывается человеком или доказательством.
  {
    name: 'checklist_add',
    description: 'Добавить пункт в чек-лист проекта — то, что важно не забыть. Пункт виден пользователю во вкладке «Чеклист» и переживает перезапуск. Используй для контрольных пунктов по ходу работы; для оркестрации внутри прогона есть todo_create.',
    parameters: {
      type: 'object',
      properties: {
        text: { type: 'string', description: 'Формулировка пункта — коротко и проверяемо.' },
        planId: { type: 'number', description: 'Опционально: связь с планом.' },
        planStepId: { type: 'number', description: 'Опционально: связь с конкретным шагом плана.' },
      },
      required: ['text'],
    },
  },
  {
    ...
    description: 'Закрыть пункт чек-листа. ТОЛЬКО с доказательством: путь к файлу, выполненная команда, ссылка на результат. Без доказательства пункт не закрывается — совпадения текста недостаточно.',
    parameters: {
      type: 'object',
      properties: {
        id: { type: 'number', description: 'Идентификатор пункта (из checklist_list).' },
        evidence: { type: 'string', description: 'Чем подтверждается выполнение: файл, команда, артефакт.' },
      },
  ...
  {
    name: 'checklist_list',
    description: 'Показать чек-лист проекта: открытые и закрытые пункты, источник (пользователь или Verstak), связь с планом и доказательства. Read-only.',
    parameters: { type: 'object', properties: {} },
  },
  {
    name: 'orchestrate',
