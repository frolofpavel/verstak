...
const RAYNER_NOTES: ReleaseNote[] = [
  {
    version: '2.0.7',
    name: 'Verstak 2.0.7',
    publishedAt: '2026-07-15T12:45:35Z',
    body: [
      '### Модели и провайдеры',
      '- Добавлен единый каталог провайдеров и живой Model Doctor для Grok: сохранённая модель честно проверяется по доступному каталогу.',
      '- Маршрут модели на один prompt: можно выбрать провайдера/модель для следующей отправки без смены дефолта чата.',
      '- При fallback сохраняются requested и actual провайдер/модель, чтобы было видно, что пользователь выбрал и что реально отработало.',
      '',
      '### Агент и стабильность',
      '- Неизвестные инструменты обрабатываются явно, без generic-пути с пустыми аргументами.',
      '- Улучшен auto-compact длинной истории и прогресс ожидания модели.',
      '',
      '### Интерфейс',
      '- Настройки провайдеров показывают live/bundled состояние каталога и устаревшие модели без тихой подмены.',
    ].join('\n'),
    htmlUrl: 'https://github.com/frolofpavel/verstak/releases/tag/v2.0.7',
  },
  {
    version: '1.5.5',
    name: 'Verstak 1.5.5',
