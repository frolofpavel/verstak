...
import { mkdirSync, readFileSync, existsSync as fsExistsSync } from 'fs'
...
app.whenReady().then(() => {
  ...
  const aiIpcGateway = registerAiIpc(aiDeps)
  aiGateway = aiIpcGateway
  const { invokeAiSend } = aiIpcGateway
