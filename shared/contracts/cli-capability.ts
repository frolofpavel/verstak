// Единый источник правды о том, что CLI-провайдеры РЕАЛЬНО умеют под контролем
// Verstak. Раньше эти факты дублировались между renderer (src/lib/runtime-capability.ts)
// и main (electron/ai/control-envelope.ts + electron/ai/cli-security-capabilities.ts)
// и держались синхронными вручную под анти-дрейф-тестами. Теперь — одна правда,
// импортируемая обеими сторонами, расходиться нечему.
//
// Файл намеренно БЕЗ рантайм-зависимостей (ни electron, ни node, ни react): его
// импортируют обе стороны — как и остальные shared/contracts.

// ─── Проекция tool-таймлайна ────────────────────────────────────────────────
// CLI-провайдеры, где проекция родного tool-таймлайна РЕАЛЬНО реализована и
// проверена тестами (срез 1 — claude-cli, срез 2 — codex-cli). Набор намеренно
// узкий: не добавлять сюда провайдер, пока проекция не подтверждена на потоке.
export const CLI_WITH_TIMELINE: ReadonlySet<string> = new Set(['claude-cli', 'codex-cli'])

// ─── Защита секретов по CLI-провайдерам (1.9.6 #2) ───────────────────────────
//
