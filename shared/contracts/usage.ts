...
/**
 * ЗАДАЧА A (штаб, 06.08): ставка $/1M за КЭШИРОВАННЫЙ input-токен.
 *
 * Правило НЕСУЩЕЕ: нет официальной цены кэша у модели → считаем кэш по ПОЛНОЙ цене
 * input, а НЕ по нулю. Занижение опаснее завышения: контроллер расхода существует
 * ради порогов ($2 жёлтый / $5 красный), и счётчик-занижатель срабатывает ПОЗЖЕ,
 * чем должен, — односторонний ущерб. Завышение безобидно: человек видит потолок.
 * Никогда не считать кэш бесплатным по умолчанию. Единое место на оба слоя
 * (renderer pricing.ts + main cost-guard.ts), чтобы правило не разошлось.
 *
 * `cachedPrice === 0` (легитимно бесплатный кэш из документации провайдера) уважаем:
 * проверка по `!= null`, НЕ по truthiness — иначе честный ноль свалился бы в
 * «неизвестно» и кэш посчитался бы по полной, хотя провайдер его дал даром.
 */
export function cachedTokenRate(cachedPrice: number | null | undefined, inputPrice: number): number {
  return cachedPrice != null ? cachedPrice : inputPrice
}
