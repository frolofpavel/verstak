...
export const RUNTIME_FLAG_DEFAULT_ON: Record<RuntimeFlagKey, boolean> = {
  ...
  /**
   * opt-in СОЗНАТЕЛЬНО. Губернатор доверия ужесточает решения по уровню
   * возможности, а уровень без доказательств — T1 («только чтение»). Включи
   * дефолтом — и КАЖДЫЙ прогон с любым из сотен установленных скиллов начнёт
   * спрашивать подтверждение на каждую правку, включая авто-режим. Это не
   * улучшение безопасности, а поломка продукта у всех, кто ничего не просил:
   * человек выбрал режим сознательно, и молча переопределять его выбор нельзя.
   * Механизм построен и доказан; включение — отдельное решение Павла.
   */
