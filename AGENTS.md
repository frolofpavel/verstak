...
- **Корень репозитория не является папкой handoff.** Одноразовые `TASK-*.md`,
  отчёты агента и временные security/audit notes класть в `Downloads`; долговечную
  документацию — только в `docs/` с записью в `docs/README.md`. Не создавать
  новые task/report-файлы рядом с `package.json`.
...
Если задача относится к мобильному приложению, связи телефона с desktop Verstak, relay, pairing или удалённому запуску чатов, сначала прочитать `mobile/AGENTS.md`. Там зафиксированы цель, текущая реализация, проверки, ограничения безопасности и дальнейший план.
