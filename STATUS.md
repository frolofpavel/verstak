...
**Обновлено:** 2026-06-22 · **В проде:** `1.5.27` · **Тесты:** 1378 зелёных · **Ветка:** main

> **1.5.27** — стабильность по итогам конкурентного код-ревью (OpenCode/KiloCode): дедуп дублей сообщений в БД (5.2), полный сброс состояния при закрытии проекта (5.3), безопасный фолбэк провайдера с накопленной историей (6.2), рефактор ядра агент-цикла в типизированный `AgentRunContext` вместо 34 позиционных параметров (6.1). Все по TDD.
