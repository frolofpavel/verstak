...
**Обновлено:** 2026-06-24 · **В проде:** `1.5.32` · **Тесты:** ~1490 зелёных (полный прогон) · **Ветка:** main · **В работе:** усиление по конкурентному исследованию (T1.5 ✓ в main, не зарелизено)
...
- ⬜ **T1.4 PTC** (`execute_code`, тулзы по RPC, в окно только stdout) — кратный рычаг на «Opus на дешёвой» (меньше токенов). L, нужен sandbox-дизайн.
- ⬜ **T1.1 мультиязычный per-file LSP в петле** (оживить `lsp/client.ts`, pyright/gopls + per-file инжект в tool_result) — закрыть не-TS дыру (РФ на Python/Go/1C). L.
- ⬜ T1.3 Inbox approvals по всем чатам (M) · T1.6 структурная итеративная компакция (L) · T1.2 git-worktree изоляция swarm (M).
- 💡 Новые фишки: TG-двусторонний-пульт к Gateway (статус/approve/Proof + воронка), Proof Pack как юр-артефакт сдачи, PTC-витрина «стоимость хода».
