...
**Обновлено:** 2026-06-23 · **В проде:** `1.5.29` · **Тесты:** 1420 зелёных (полный прогон) · **Ветка:** main
...
**Зарелизено в `1.5.18`** (19.06 ночь), сейчас в проде `1.5.26`. Харнес A–E **прокликан вживую 19.06 и работает** (`TASK-ilya-v3-livetest.md`). Илья получает через автообновление.
...
### 🆕 Хвосты на следующую сессию (записано 23.06, после длинной сессии)

| # | Что | Детали | Зона |
|---|-----|--------|------|
| A | **Gateway-баги → отдать Codex** | Пакет с file:line+фиксами: `Downloads/IRI-Gateway-баги-ревью-2026-06-23.md`. Начать с 🔴 двойного начисления баланса + овердрафта (прямые деньги). Память: [[review-2026-06-23]] | Codex |
| B | ~~21 medium/low находка ревью~~ — **Verstak-часть разобрана 23.06 ч.2** (см. Сделано). 3 названных + 3 новых из ре-ревью пофикшены TDD; #2 by-design. Gateway-часть (cache-race, auth_reset bypass) → Codex | ✅ Verstak |
| C | **Решение по архитектуре скиллов** (обсуждали 23.06) | Варианты: (1) аудит 50+ скиллов — что живо/мёртво/схлопнуть; (2) pre-commit хук Verstak (блок коммита при падении type/тестов = гарантия вместо обещания); (3) тонкий `/verstak`-скилл (авто-STATUS + журнал + реестр задач). Пока НЕ решено — выбор Павла | Павел |

> **Контекст 23.06:** проведена огромная сессия — gateway переоценён по официальным ценам (41 модель, формула `база$×ЦБ×1.03×1.11×1.30`, см. [[iri-gateway-backend]]), 6 провайдеров заведено (Gemini/Grok/GLM/MiniMax/Kimi/Qwen), Verstak **1.5.28** зарелизен (3 фикса по ревью: SSH-secret-bypass crit + утечка состояния + неполный откат). Всё в проде + памяти.
...
**Догфудинг — ревью самого Verstak (вкладка Code-review) нашёл 3 находки в той же области:**
- **finding 3 (Low) — ПОФИКШЕН (не зарелизен, поедет в следующий релиз):** эфемерные `preflights`/`subagentRuns` не входят в bundle и не сбрасывались на смене чата → карточки активности уходящего чата залипали. Сброс на всех context-switch сайтах (switchChatSession обе ветки + newChatSession + setProject), как уже делается с touchedFiles/checkpointId/artifacts. +2 теста (project-store-lifecycle). type ✓, store 55 зелёных.
- **finding 2 (Low-Med) → бэклог §5 PerChatState:** checkpointId зануляется на restore чата НАМЕРЕННО (anti-leak фикс 1.5.28) — побочка: кнопка отката пропадает при переключении чатов (per-file undo жив). Правильный фикс — внести checkpointId/preflights/subagentRuns в `ChatStateBundle` (preserve per-chat), но это §5-рефактор: перенос типов PreflightCard/SubagentRunCard + переписать локирующие тесты + живой UI-тест. Не пост-релизный патч.
- **finding 1 (Low-Med) → бэклог durability:** undo `floors` (FloorTracker) in-memory — теряется при краше (узкий edge: чекпоинт + >50 правок + краш + ещё правки + откат). Пре-существующее (исходный Map был таким же). Фикс — persist floors в SQLite-таблицу `undo_floors` + восстановление при старте.
