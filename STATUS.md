...
**Обновлено:** 2026-06-20 · **В проде:** `1.5.22` · **Тесты:** 1280 зелёных · **Ветка:** main
...
- **Покрытие хендлеров (§5.3, после распила)** — +27 тестов на критичные рубежи: ядро DoD `attest_verification` (статус по exitCode, сверка claimed-vs-actual, 8), гейтинг записи `diffConfirmWrite` (block/auto/confirm, 6), гейтинг команд `run_command` + редакция секретов (6), `connector_query` + guard Я.Диск (7). Все side-effect-поверхности гейтятся протестированно. **Тестов: 1280.**
