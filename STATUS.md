...
**Обновлено:** 2026-06-26 · **В проде:** `1.5.34` 🚀 · **В main (не зарелизено):** Tier-2 #1 LSP-навигация (find_definition/find_references) · **Тесты:** ~1550 зелёных · **Ветка:** main · **Хвост:** T1.2 деферн (дизайн в роудмапе); Rust/Go LSP у Pavel — env (`rustup component add rust-analyzer` + gopls)
...
- 🧭 **Tier-2 (дифференциаторы) — начат:** ✅ **#1 LSP-навигация** (`find_definition`/`find_references`, семантика через язык-сервер; в main `70fca63`→`254daa2`, не зарелизено; TDD + live pyright + ревью 6 багов закрыто). Осталось: #2 microcompact (M), #3 дерево сессий branch/fork (L), #4 bash-allowlist + permission-движок (M), #5 live-browser + MP4 в Proof (M), #6 file-scoped правила `.verstak/rules/*.mdc` (S, дёшево).
