...
**Обновлено:** 2026-06-24 · **В проде:** `1.5.32` · **Тесты:** ~1528 зелёных (полный прогон) · **Ветка:** main · **В работе:** усиление по исследованию — **5/6 Tier-1 в main** (T1.5/T1.6/T1.3/T1.1/T1.4 ✓, T1.2 деферн), не зарелизено
...
Реализовано автономно по TDD за сессию 24.06 (каждый коммит через pre-commit гейт, все в main, НЕ зарелизено — ждёт сборки + live-проверки не-TS/PTC):
...
- ⏸ **T1.2 git-worktree изоляция swarm** — ДЕФЕРНУТО с дизайном. Реальный баг (параллельные executor'ы свопа бегут с общим `cwd: ctx.projectPath` → клобберят диск), но фикс = L-переработка модели синтеза арбитра (messages→diffs) + re-root FileTools per-worktree + cleanup, **не верифицируется автономно**. Дизайн: `git-worktree.ts` (add/remove/list) → каждый executor в своём worktree → арбитр сравнивает `git diff` → применяет выбранный в main → cleanup. Делать в отдельной сессии с live-проверкой роя. (Не строю орфан-хелпер сейчас — минимализм.)
- 🔍 **Адверсариальное ревью всех 5 фич** (workflow 19 агентов × верификация каждой находки) → **9 реальных багов из 14, все закрыты по TDD:** 🔴CRIT PTC vm-побег → RCE (vm не песочница; фикс: гейтинг execute_code как команда = trust run_command + хардинг vm + честные комменты, `59e6601`); 🟠LSP spawn без shell → ENOENT на Windows (фича была мертва на осн. платформе) + EPIPE-краш main + мульти-файл (`55b1c0d`); 🟠T1.5 Format 5 ложно срабатывал на прозе с JSON → фантомный вызов (фикс: валидация имён против tools + сужение парсера, `e90ac10`); 🟠T1.6 extractTouchedFiles пропускал propose_edits (`1c15471`); T1.3 ghost-approval + sendId (`6523197`). **Урок: ревью окупилось — нашло critical-RCE и «фича мертва на Windows», которые нельзя поймать без live/адверсариально.**
