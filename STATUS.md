...
**Обновлено:** 2026-06-26 · **В проде:** `1.5.34` 🚀 · **В main (не зарелизено):** Tier-2 #1/#6/#4/#2 (LSP-навигация, file-scoped rules, bash-allowlist, microcompact) · **Тесты:** ~1590 зелёных · **Ветка:** main · **Хвост:** #5/#3 дизайн+деферал (UI/integration, не верифицируются автономно); T1.2 деферн
...
- 🧭 **Tier-2 (дифференциаторы) — 4/6 в main** (TDD → адверсариальное ревью → фиксы; не зарелизено):
  ...
  - ⏸ **#5 live-browser + MP4 → Proof** — ДИЗАЙН+ДЕФЕРАЛ. Есть browser_navigate/screenshot + ffmpeg + Proof-инфра. Добавить: `browser_click` (рендерер `verstakBrowser.click` + handler + def), авто-цикл скриншотов в Pipeline-verify → ffmpeg в MP4 → в Proof Pack. **Не верифицируется автономно** (браузер + видео + визуал) — нужна live-сессия с Browser-вкладкой.
  - ⏸ **#3 дерево сессий (branch/rewind/fork)** — ДИЗАЙН+ДЕФЕРАЛ. Поверх готовых checkpoint+undo+resume + parent_chat_id. L-размер, **UI-тяжёлый** (визуализация дерева веток) — не верифицируется headless. Нужна отдельная сессия с live-UI.
