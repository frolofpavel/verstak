...
Автономные срезы надёжности длинных сессий (Mode C session-continuity отложен — нужен живой claude для верификации).

### Исправлено
- **Меньше износа БД на длинных сессиях.** Снапшот для восстановления писался целиком на каждом ходе; теперь пропускается при отсутствии изменений и разрежается на длинных прогонах (без потери восстановимости).
- **Устойчивость точки отката.** Снапшот грязных правок перед CLI-прогоном закрепляется так, что его не выгребает `git gc`; откат честно показывает, жив ли снапшот.
- **Защита фоновых cron-прогонов.** Тест-страж гарантирует, что unattended-прогон получает только read-only инструменты — регрессия с write-инструментом теперь падает.
