...
Последнее обновление: 2026-08-15, вторым заходом (остаток ревизии `docs/revision-2026-08.md`, §3.2 и §3.3): защита секретов сведена в один источник — CLI-путь перестал держать свою редакцию списка, которая была СЛАБЕЕ API-пути; число провайдеров убрано из публичных текстов совсем (решение Павла), константа `MODEL_PROVIDER_COUNT` удалена. Обе записи «не закрыто и ждёт работы» в §5 сняты тем же движением, эталон тестов — 5596.
