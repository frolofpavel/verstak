...
- **Прогон, собравший МЕНЬШЕ тестов, чем эталон, — не вердикт, а оборванный
  запуск.** Эталон на 30.07: `numTotalTests` = **4501**
  (4487 passed + 14 skipped); до него 4497, 4487, 4479, 4469, 4448, 4426, 4418, 4406, 4400, 4395, 4385, 4377, 4370, 4364, 4358, 4353, 4348, 4321, 4310, 4290, 4285, 4278, 4274, 4267, 4256, 4237, 4212.
...
правок:
- **30.07 (A3 §2.2):** одна директива в шаг PLAN — многошаговое исследование
  (аудит, разбор, сводка из нескольких источников) вызывает `create_plan`, даже
  если работа только читает; такой план информационный и не ждёт согласования.
  Повод: цикл планов включён по умолчанию (§2.1), но на читающих задачах план не
  появлялся, потому что модель его не создавала (порог шага PLAN строже описания
  инструмента `create_plan` — тексты противоречили, директива сняла противоречие,
  а `tools.ts` приведён с ней в согласие). `SYSTEM_LAYER_VERSION` 1.3.1 → 1.3.2.
  **Доказуемость:** пин стережёт присутствие строки в собранном промпте от
  случайного удаления; следование модели строке тестом НЕ доказуемо — это
  просьба, а не гейт. Единственное доказательство — живой прогон. Жёсткая граница
  «когда план обязателен» существует одна — рантайм-порог §4.2 логики плана, и он
  сознательно не тронут.
