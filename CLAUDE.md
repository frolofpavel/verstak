...
**Базовые фичи:**
- **Провайдеры (число здесь УСТАРЕЛО, см. ниже):** база — Gemini API/CLI, Claude API/Code, Grok API/CLI, OpenAI API, Codex CLI, GigaChat, YandexGPT; плюс OpenAI-совместимые (DeepSeek, Qwen, Mistral, Moonshot, Groq, OpenRouter, Ollama, custom endpoint). Аккаунт упал — переключился, не теряя работу.
  Здесь стояло «18 провайдеров: база (10) + 8 OpenAI-совместимых». Факт на 15.08 (ревизия §3.3): `PROVIDER_IDS` в `shared/contracts/provider.ts` содержит **22** идентификатора (11 явных + 11 extra). То же устаревшее «18» показывает первый экран установщика — `MODEL_PROVIDER_COUNT` в `src/installer/constants.ts` с пометкой «синхронизировать руками» и без единого стража. **Какое число показывать человеку — решение Павла** (22 — технический факт, продуктовая подача может быть другой), но оно должно быть ОДНО и проверяемое: до этого решения ни один текст не вправе называть себя точным.
...
1. **~~ГЛАВНЫЙ монолит — `electron/ipc/ai.ts`~~ РАСПИЛЕН (1.9.8 #1 → пакет 2.1.10):** было ~3360 строк, стало **1104** (−67%). Оба гиганта вынесены в `ai/runner-*.ts`: `runApiConversation` (~1300) → `runner-api.ts`, `runPlainConversation` (~419) → `runner-plain.ts`; circular-dep разорван через `runner-shared.ts` (pending-registry + turn-константы), `AiDeps` type-only. Каждый переезд верифицирован харнесами `tests/ipc/agent-loop.test.ts`(30)+`plain-loop.test.ts`(17) — поведение идентично; 2 адверсариальных ревьюера подтвердили (identity разделяемого состояния + семантика байт-в-байт). `ai.ts` = IPC-shell (registerAiIpc ~700 + resolve/stop/suspend + scheduled headless). **Остаток:** registerAiIpc можно дробить дальше, но это уже НЕ гигант-функция — низкий приоритет. При правках runner'ов держать харнесы зелёными.
   ...
   - **Дубли renderer↔main — КАНДИДАТЫ НА ПЕРЕЕЗД, а не вечная данность (запись исправлена 30.07).** Здесь стояло «нет shared-модуля из-за context-изоляции», и это НЕВЕРНО: `shared/` существует и импортируется ОБОИМИ слоями (52 файла в `electron/`, 10 в `src/` — `Chat.tsx`, `useProvider.ts`, `agent-progress.ts`, `model-catalog.ts`, `pricing.ts`, `subscription-telemetry.ts`). Ложное обоснование дороже ложного факта: оно объясняло, ПОЧЕМУ дубли нельзя убрать, и потому их годами держали руками и обкладывали стражами вместо устранения. Первым переехал `plan_approval_gate` (A3 §2.1): полярность живёт в `shared/contracts/runtime-flag-policy.ts`, main больше не сравнивает строку сам — расходиться стало нечему.
     ...
     **Что действительно осталось от дублей** (по той же ревизии, §3.1–3.3, проверять фактом, а не этим абзацем): полярность рантайм-флагов сведена в один источник 15.08 — `shared/contracts/runtime-flag-policy.ts` (`RUNTIME_FLAG_DEFAULT_ON` + `runtimeFlagOn`), main и renderer читают её оттуда, возврат сравнения строки в `electron/` ловит пин в `tests/lib/runtime-flags.test.ts`. НЕ закрыто и ждёт своей работы: список секрето-путей `secret-scanner.isForbiddenPath` ↔ `claude-cli.SECRET_GLOBS` (уже РАЗОШЁЛСЯ — приватный ключ в корне проекта CLI прочитает, API-путь закроет) и `MODEL_PROVIDER_COUNT` в установщике (комментарий «синхронизировать руками», значение 18 против 22 в `PROVIDER_IDS` — врёт на первом экране).
...
Последнее обновление: 2026-08-15 (уборка по ревизии `docs/revision-2026-08.md`, разделы 1 и 3: снят список пяти «кандидатов на переезд» в §5 — переезд закончен, дублей нет; снята устаревшая цифра «18 провайдеров» в §1 — в реестре 22, решение о показываемом числе за Павлом; эталон тестов в §3.1 приведён к измеренному 5546 и объявлен снимком при живом источнике в `scripts/release-gate.mjs`).

Предыдущее обновление: 2026-07-27 (позиция 5 плана работ: §2 и §5 сверены с кодом покомандно — размеры ai.ts 1104, projectStore 1153, Chat.tsx 3205, Settings 4662, layout.css 22055; добавлены 12 модулей `components/chat/` и полный список `ai/runner-*`; снята неверная запись про 0 тестов у cross-verify; обновлены Mode C и PTC). Если архитектура изменилась — обнови этот файл.
