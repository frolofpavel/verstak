...
- **Прогон, собравший МЕНЬШЕ тестов, чем эталон, — не вердикт, а оборванный
  запуск.** Эталон: `numTotalTests` = **4779**
  (слияние двух линий перед деплоем облачного ядра: десктопная main дала +28 от
  общей базы 4736, облачная headless-line +15; пересечений по файлам тестов нет,
  поэтому приросты складываются — 4736 + 28 + 15. Число считается ВРУЧНУЮ по
  родословным в шапке `scripts/release-gate.mjs`, прогон его подтверждает, но не
  назначает);
  до него по облачной линии 4751 (graceful shutdown: host.close() ждёт живые
