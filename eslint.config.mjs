...
export default tseslint.config(
  {
    ...
    ignores: [
      'node_modules/**',
      // Рабочие деревья сессий (`git worktree` под .claude/) — ПОЛНЫЕ копии проекта.
      // Без этой строки `lint:full` обходил их наравне с исходниками: пять копий давали
      // 1124 лишних предупреждения (3124 вместо 2000) и валили сам линт по памяти
      // (`heap out of memory` на 4 ГБ, exit 134) — то есть проверка не давала вердикта
      // вовсе, а выглядела как «lint FAIL» по вине кода. Замер 13.08.
      '.claude/worktrees/**',
      'out/**',
