...
function buildInterruptedRunProgress(run: ResumableRun): AgentProgressEntry[] {
  ...
  // ЗАДАЧА 1 (b, продолжение 611dd83): баннер теперь всплывает и на прогон, умерший
  // В СЕССИИ (run-finalized перечитывает список без рестарта), а не только на
  // крах-через-рестарт. Поэтому текст НЕ утверждает «приложение было закрыто» —
  // рендер причины обрыва не знает (мог быть обрыв связи, сбой шлюза, простой
  // сокета). Говорим наблюдаемое: прогон оборвался, не завершившись.
  const detail = run.autoResumable
    ? `Verstak нашёл задачу, которая оборвалась, не завершившись. Можно повторить последний запрос с восстановленным контекстом: "${request}".${toolNote}`
    : `Модель начала работу, но прогон оборвался до безопасного завершения. Авто-продолжение с контекстом отключено (последнее действие могло менять файлы/систему), но можно запустить запрос заново — правки прерванного прогона при необходимости откати кнопкой в Инспекторе. Запрос: "${request}".${toolNote}`
