...
import type { DetectedCli, ProviderCatalogStatusDTO } from '../types/api'
    ...
    <div className="gg-settings-extra gg-external-tools-tab">
      <section className="gg-external-tools-intro">
        ...
        <div className="gg-external-tools-intro-rule">
          ...
          {/* 14.08: проброс в claude-cli построен — прежняя пометка «MCP доходит только
              до API-пути» стала ложной и снята тем же коммитом (CLAUDE.md §3.1).
              Новая говорит не про доступ, а про ГАРАНТИИ: где инструмент исполняет
              Verstak, а где чужой рантайм. Codex/Gemini/Grok CLI внешнюю конфигурацию
              per-run не принимают — разведано по их --help, см. claude-cli-mcp.ts */}
          <p className="gg-external-tools-cli-note">Подключённые здесь серверы видят API-провайдеры и Claude Code (CLI). Разница в гарантиях: на API-пути инструмент выполняет сам Verstak и применяет свои правила, на пути Claude Code его выполняет CLI своим механизмом прав — Verstak передаёт туда режим агента и запрет секретных файлов. Из этого следует предел: в режимах, где CLI спрашивает разрешение, спросить в фоне не у кого, и на пути Claude Code такой инструмент отработает только в режиме «Без ограничений». Codex CLI, Gemini CLI и Grok CLI работают своим набором инструментов и эти серверы не подхватывают</p>
        </div>
      </section>
                ...
                {catalog.filter(c => c.group === group).map(c => {
                  ...
                  return (
                    <article key={c.id} className={`gg-mcp-catalog-card ${open ? 'is-open' : ''}`}>
                      ...
                      <p>{c.description}</p>
                      {/* Один сервис бывает доступен двумя путями, и разница не
                          косметическая: свой коннектор читает и только читает, ключи
                          лежат в safeStorage, вызов исполняет сам Verstak по своим
                          правилам; сервер — чужой код, который может уметь запись и
                          исполняет себя сам. Молчать об этом хуже, чем не добавлять
                          сервер вовсе: человек выбирает вслепую. */}
                      {c.ownConnector && (
                        <p className="gg-mcp-catalog-own">
                          У Verstak есть свой коннектор «{c.ownConnector}»: он только читает,
                          ключи хранит у себя и работает со всеми провайдерами. Этот сервер —
                          сторонний, шире по возможностям, но и правила у него свои.
                        </p>
                      )}
...
export function Settings({ onClose, initialTab }: { onClose: () => void; initialTab?: Tab }) {
  ...
  const TAB_GROUPS: ReadonlyArray<SettingsNavGroup> = [
    ...
    { title: 'AI', tabs: [
      ...
      { id: 'runtimeFlags', label: 'Поведение агента', icon: 'runtimeFlags', keywords: 'память проекта memory lifecycle автозахват auto capture умный выбор модели smart routing автоподмена fallback знание о проекте project brain флаги поведение переключатели тумблеры включить выключить согласование плана одобрение плана ждать одобрения plan approval gate' },
