...
export const RUNTIME_FLAGS: readonly RuntimeFlagDef[] = [
  ...
  {
    key: 'capability_trust_gate',
    // Подпись говорит, что человек ПОЛУЧАЕТ, а не как это устроено внутри.
    title: 'Ограничивать возможности по их истории',
    what: 'Скилл, который ещё ничего не доказал, работает как «только чтение»: правки и команды спрашивают подтверждение, даже в авто-режиме. Права возвращаются по мере пройденных проверок и принятых тобой результатов. Уровень каждой возможности виден на экране «Скиллы» → «Реестр».',
    whenOff: 'Скиллы работают по выбранному тобой режиму, как раньше. Реестр всё равно показывает уровень доверия — просто он ни на что не влияет.',
    // Выключено по умолчанию СОЗНАТЕЛЬНО: включение меняет поведение каждого
    // прогона с любым скиллом, и это решение человека, а не побочный эффект
    // обновления.
    defaultOn: RUNTIME_FLAG_DEFAULT_ON.capability_trust_gate,
    readAt: 'electron/main.ts',
  },
] as const
