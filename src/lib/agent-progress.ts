...
export function reduceAgentProgress(
  ...
  if (event.type === 'thought') {
    let next = markAgentProgress(progress, ['model'], 'done')
  ...
  if (event.type === 'text') {
    let next = markAgentProgress(progress, ['context', 'model', 'reasoning'], 'done')
    return upsertAgentProgress(next, {
      id: 'final',
      phase: 'final',
  ...
  if (event.type === 'done') {
    let next = finishRunning(progress, 'done')
    return upsertAgentProgress(next, {
      id: 'done',
      phase: 'final',
      title: 'Ответ готов',
  ...
  if (event.type === 'error') {
    let next = finishRunning(progress, 'error')
    return upsertAgentProgress(next, {
      id: 'error',
      phase: 'final',
