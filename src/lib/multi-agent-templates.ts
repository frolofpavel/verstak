 ...
 *
 * ЕДИНСТВЕННАЯ точка входа в UI (цель 2.7.0, пункт 2): скрытые системные
 * slash-команды /orchestrate, /swarm, /parallel — src/components/chat/
 * system-slash-commands.ts. Они не предлагаются в списке и всплывают только при
 * осознанном наборе: это отладочный путь, а не кнопка.
 *
 * Прежде входов было три — здесь же перечислялись пункт «Мультиагент» в меню
 * «Инструменты» и quick-action кнопки пустого состояния панели «Агенты». Оба
 * сняты: они НИЧЕГО не запускали, а вставляли этот текст в поле ввода, тогда как
 * инструменты orchestrate/swarm/delegate_parallel живут у агента и вызываются им
 * самим. Кнопка давала не возможность, а формулировку.
 *
